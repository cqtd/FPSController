...
namespace Cqunity
{
	public class InputUtility
	{
		...
		public static void DisplayMissingInputs(int InputFlags)
		{
			...
			{
				DisplayText(
					$"To control the character with joystick, please configure '{MoveHorizontalAxis}' and '{MoveVerticalAxis}' axes in the Input Manager.");
			}

			if ((InputFlags & CameraInput) > 0 && !AreCameraInputsSetup())
			{
				DisplayText(
					$"To control the camera with joystick, please configure '{CameraHorizontalAxis}' and '{CameraVerticalAxis}' axes in the Input Manager.");
